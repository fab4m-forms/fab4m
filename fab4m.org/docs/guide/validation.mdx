...
```jsx
import { generateSchema } from "fab4m";
generateSchema(form);
```
...
```jsx
import { generatePartSchemas } from "fab4m";
...
Having a JSON schema readily available for any form means that you can always validate the input from any form on any backend with a JSON validator available.
